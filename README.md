...
```bash
lambda-using-aws-parameter-store$ sam build
```
...
```bash
lambda-using-aws-parameter-store$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-using-aws-parameter-store$ sam local start-api
lambda-using-aws-parameter-store$ curl http://localhost:3000/
```
...
```bash
lambda-using-aws-parameter-store$ sam logs -n HelloWorldFunction --stack-name lambda-using-aws-parameter-store --tail
```
...
```bash
lambda-using-aws-parameter-store$ cd hello-world
...
```bash
sam delete --stack-name lambda-using-aws-parameter-store
```
